...
<body>
	...
	$sql = "SELECT * FROM achieve WHERE category='lifegoal'";
	...
	while($row = mysqli_fetch_array($result)){
	    if($row['achieve']==0)
	        echo "<tr><td><a href=\"view_lifegoal.php?num={$row['num']}\">{$row["title"]}</a> </td><td> {$row["create_date"]} </td></tr>";
        }  
	echo "</table>";
	...
	while($row = mysqli_fetch_array($result)){
	    if($row['achieve']==1)
                echo "<tr><td><a href=\"view_lifegoal_com.php?num={$row['num']}\">{$row["title"]}</a> </td><td> {$row["create_date"]} </td><td> {$row["complete_date"]} </td></tr>";
        }
        echo "</table>";
    ...
    ?>
    <br>
    <a href = "write_lifegoal.php">add new</a>
